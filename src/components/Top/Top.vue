<template>
    <div class="header">
        <div class="content-wrap">
            <div class="avatar">
                <img width="64" height="64" :src="seller.avatar">
            </div>
            <div class="content">
                <div class="title">
                    <span class="brand"></span>
                    <span class="name">
                        {{seller.name}}
                    </span>
                </div>
                <div class="description">
                    {{seller.description}}/{{seller.deliveryTime}}分钟送达
                </div>
                <div v-if="seller.supports" class="support">
                    <span class="icon"></span>
                    <span class="text">{{seller.supports[0].description}}</span>
                </div>
                
            </div>
            <div v-if="seller.supports" class="support-count" @click="showDetail">
                    <span class="count">
                        {{seller.supports.length}}
                    </span>
                    <i class="icon-keyboard_arrorw_right">
                    </i>
                </div>
        </div>
        <div class="bulletin-wrapper" @click="showDetail">
            <span class="bulletin-title">
            </span>
            <span class="bulletin-text">
                {{seller.bulletin}}
            </span>
            <i class="icon-keyboard_arrow_right">
            </i>
            <span>
            </span>
        <div class="background">
            <img :src="seller.avatar" width="100%" height="108px" >
        </div>
        <div v-show="detailShow" class="detail">
            <div class="detail-wrap" clearfix>
                <div class="detail-main">
                    <h1>{{seller.name}}</h1>
                    <div class="star-wrap">
                        <star :size="36" :score="seller.score"></star>
                    </div>
                    <div class="title">
                        <div class="line"></div>
                        <div class="text">优惠政策</div>
                        <div class="line"></div>
                    </div>
                    <ul v-if="seller.supports" class="supports">
                        <li class="support-item" v-for="(item,index) in seller.supports">
                            <span class="icon" :class="classMap[seller.supports[index].type]"></span>
                            <span class="text">{{item.description}}</span>
                        </li>
                    </ul>
                    <div class="title">
                        <div class="line"></div>
                        <div class="text">商家公告</div>
                        <div class="line"></div>
                    </div>
                    <div class="bulletin">
                        <p class="content">
                            {{seller.bulletin}}
                        </p>
                    </div>
                </div>
            </div>
            <span class="icon-close" @click.stop="closeDetail"><img class="close-image" src="../../common/images/close.png" width="32px" height="32px"></span>
        </div>
    </div>
</div>
</template>
<script>
import Star from '@/components/Star/Star';

    export default{
        props:{
            seller:{
                type:Object
            }
        },
        data(){
            return{
                detailShow:false,
                isActive: true
            }
        },
        methods:{
            showDetail(){
                this.detailShow=true;
                console.log(1);
            },
            closeDetail(){
                this.detailShow=false;
                console.log(2);
            }
        },
        created(){
            this.classMap = ['decrease','discount','special','invoice','guarantee'];
        },
        components:{
            'star':Star
        },
        computed:{
            classObject: function () {
                console.log(classObject);
                return {
                active: this.isActive
                }
            }
        }
       
    }
</script>
<style scoped>
body{
    padding:0;
    margin:0;
}
h1{
    margin-bottom:0;
}
.header{
    height:100;
    color:#fff;
    background:rgba(7,17,27,0.1);
}
.header .content-wrap{
 
    position:relative;
    padding:24px 12px 18px 24px;
    font-size:0;
}

.header .content-wrap .avatar{
    display:inline-block;
    vertical-align:top;
}
.header .content-wrap .content{
    display:inline-block;
    margin-left:16px;
    font-size:14px;
}
.header .content-wrap .content .title{
    margin:2px 0 8px 0;
}
.header .content-wrap .content .title .brand{
    vertical-align:top;
    width:30px;
    height:18px;
    display:inline-block;
   background-image:url("./brand@2x.png");
   background-size:30px 18px;
   background-repeat:no-repeat;
        
    
}
.name{
    font-size:16px;
    margin-left:6px;
    line-height:18px;
    font-weight:block;
}
.description{
    font-size:12px;
    line-height:12px;
    margin-bottom:10px;
}
.support .icon{
    display:inline-block;
    width:12px;
    height:12px;
    margin-right:4px;
    background-size:12px 12px;
    background-repeat:no-repeat;
}
.text{
    line-height:12px;
    font-size:12px;
}
.support-count{
    position:absolute;
    right:12px;
    bottom:14px;
    padding:0 8px;
    height:24px;
    line-height:24px;
    border-radius:10px;
    background:rgba(0,0,0,0.2);
    text-align:center;
  

}
.count{
    font-size:10px;
    vertical-align:top;

}
.icon-keyboard_arrorw_right{
    line-height:24px;
    font-size:10px;
    margin-left:2px;
}

.bulletin-wrapper{
    height:28px;
    line-height:28px;
    padding:0 22px 0 12px;
    overflow:hidden;
    text-overflow:ellipsis;
    background:rgba(7,17,27,0.2);

}
.bulletin-text{
    vertical-align:top;
    margin:0 4px;
    font-span:10px;
}
.background{
    position:absolute;
    top:0;
    left:0;
    width:100%;
    height:108px;;
    z-index:-1;
    filter:blur(10px);
}
.detail{
    position:fixed;
    top:0;
    left:0;
    z-index:100;
    width:100%;
    height:100%;
    background:rgba(7,17,27,0.8);
    overflow:auto;

}
.detail-wrapper{
    min-height:100%;

}
.detail-wrapper .detail-main{
   margin-top:64px;
   padding-bottom:64px;
    
}
.close-image{
   position:absolute;
   bottom:32px;
   margin:0 auto;
    
}
.clearfix{
    display:inline-block;

}
.star-wrap{
    main-top:18px;
    padding:2px 0;
    text-align:center;
}
.title{
    display:flex;
    width:80%;
    margin:30px auto 24px auto;
}
.line{
    flex:1;
    position:relative;
    top:-6px;
    border-bottom:1px solid rgba(255,255,255,0.2);
}
.text{
    padding:0 12px;
    font-size:14px;
}
.supports{
    width:80%;
    margin:0 auto;
}
ul li{
     list-style-type:none;
}
.support-item{
    padding:0 12px;
    font-size:0;
   float:left;

}
.icon{
    display:inline-block;
    width:16px;
    height:16px;
    vertical-align:top;
    margin-right:6px;
    background-size:16px 16px;
    background-repeat:no-repeat;
}
.decrease{
    background-image:url('decrease_2@2x.png');
}
.discount{
    background-image:url('discount_2@2x.png');
}
.special{
    background-image:url('special_2@2x.png');
}
.invoice{
    background-image:url('invoice_2@2x.png');
    
}
.guarantee{
    background-image:url('guarantee_2@2x.png');
}
.supports-item .text{
    font-size:12px;
    line-height:12px;
}
.bulletin{
    width:80%;
    margin:0 auto;
}
.content{
    padding:0 12px;
    line-height:24px;
    font-size:12px;
}



</style>